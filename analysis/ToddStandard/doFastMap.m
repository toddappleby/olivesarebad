function [staBlue,staYellow,dataAll,frameValues,frameValuesContinuous,frameTimesContinuous,staPerCheckBlue,staPerCheckYellow,fv1] = doFastMap(r,numXChecks,numYChecks,numYStixels,numXStixels,numFrames,seeds,timings)
...
ftBlue = zeros(numYChecks, numXChecks, numFrames*binsPerFrame);
ftYellow = zeros(numYChecks, numXChecks, numFrames*binsPerFrame);
for m = 1 : length(seeds)
   
    
    [fv1,frameValues,~,fv2] = getFastNoiseFrames(numXStixels,numYStixels, numXChecks, numYChecks, 'BY', numFrames, 2, seeds(m));
    ...
    min(numFrames,length(frameTimes(fstart:end))-1)
...
%             size(conj(fft(squeeze(frameValues(:,n,p)))))
            ...
            ftBlue(n,p,:) = squeeze(ftBlue(n,p,:)) + fft(bdata(:)) ...
            ...
            if n == 24 && p == 24
                ...
                tester = ftBlue(n,p,:);
                staPerCheckBlue(m,:) =ifft(tester);
            elseif n == 24 && p == 23
                tester = ftBlue(n,p,:);
                staPerCheck2Blue(m,:) =ifft(tester);
            end
        end 
    end
...
%             size(conj(fft(squeeze(frameValues(:,n,p)))))
            ...
            ftYellow(n,p,:) = squeeze(ftYellow(n,p,:)) + fft(bdata(:)) ...
                .* conj(fft(squeeze(fv1(n,p,:))));
            
            if n == 24 && p == 24
                
             
                tester = ftYellow(n,p,:);
                staPerCheckYellow(m,:) =ifft(tester);
            elseif n == 24 && p == 23
                tester = ftYellow(n,p,:);
                staPerCheck2Yellow(m,:) =ifft(tester);
            end
        end 
    end
...
staBlue = zeros(numYChecks,numXChecks,30);
    for k = 1 : numYChecks
        for m = 1 : numXChecks
            tmp = ifft(squeeze(ftBlue(k,m,:)));
%              plot(tmp(1:30))  
%              pause
            staBlue(k,m,:) = tmp(1 : 30);
        end
    end
...
staYellow = zeros(numYChecks,numXChecks,30);
    for k = 1 : numYChecks
        for m = 1 : numXChecks
            tmp = ifft(squeeze(ftYellow(k,m,:)));
%              plot(tmp(1:30))  
%              pause
            staYellow(k,m,:) = tmp(1 : 30);
        end
    end
