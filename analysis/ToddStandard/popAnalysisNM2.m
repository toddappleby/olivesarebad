...
%all seeds
seqMean(d) = mean(mean(sum(spikingData(seqIndex,5000:end),2),2));
randMean(d) = mean(mean(sum(spikingData(randIndex,5000:end),2),2));
staticMean(d) = mean(mean(sum(spikingData(staticIndex,5000:end),2),2));
...
count=count+1;
     ...
     seqHzError(count) = sem(pcChangeHzSequential);
      ...
      hold on; plot(count+.2,mean(pcChangeHzRandom),'b.','MarkerSize',30)
       xlabel('cell type')
    ...
    seqGainError(count) = sem(pcChangeGainSequential);  
      ...
      %rand static
       ...
       hold on; plot(count+.2,mean(pcChangeGainRandom),'b.','MarkerSize',30)
       xlabel('cell type')
