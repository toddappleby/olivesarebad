function results = spikeDetectorOnlineAdree(D,desiredSTD,thresh,sampleRate)
%For online analysis
...
if (nargin<4) % trying threshold because many spikes detected in single event using std
    sampleRate = 1e4;
end
...
for i=1:Ntraces
    ...
    if isempty(thresh)
        % thresh = max(trace)/3;
        thresh = mean(trace) + desiredSTD * std(trace);
        %thresh = mean(trace(startInt)) + desiredSTD * std(trace(startInt));
    end
    ...
    %if mean((peaks)) < mean((nonspike_peaks)) + desiredSTD * ...
    %        std((nonspike_peaks)) % avg spike must be 4 stdevs from average non-spike, otherwise no spikes
    %    peak_times = [];
    %    peaks = [];
    %end
    %%%
